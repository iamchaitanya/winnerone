...
const MANHATTAN_RULES = [
    'Meaning of the word will be asked',
    '30 words per calendar month',
    'Reset every month',
...
export const Manhattan500View: React.FC<Props> = ({ onBack }) => {
    ...
    const finishGame = useCallback(async (result: VocabResult) => {
        ...
        if (selectedUser) {
            ...
            await supabase.from('manhattan500_logs').insert({ player_id: pid, score: result.score, wrong_count: result.wrongCount, earnings, details: { questions: result.questions.map(q => ({ word: q.word, meaning: q.meaning, type: q.questionType, correct: q.isCorrect })) }, played_at: playedAt.toISOString() });
    ...
    const { currentQuestion, currentIndex, questions, score, wrongCount, timeLeft, answered, isCorrectAnswer, startGame: triggerStart, handleAnswer } = useVocabEngine(manhattan500 as VocabWord[], 30, 'manhattan500', finishGame, true);
