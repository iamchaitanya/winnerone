...
import { useDivideEngine, DivQuestionResult } from '../src/hooks/useDivideEngine';
import { DivideHub } from '../src/components/divide/DivideHub';
import { DividePinEntry } from '../src/components/divide/DividePinEntry';
import { DividePreEntry } from '../src/components/divide/DividePreEntry';
import { DivideQuiz } from '../src/components/divide/DivideQuiz';
import { DivideResults } from '../src/components/divide/DivideResults';
import { DivideDashboard } from '../src/components/divide/DivideDashboard';
import { DivideReview } from '../src/components/divide/DivideReview';
import { DivideHistory } from '../src/components/divide/DivideHistory';

interface DivideViewProps { onBack: () => void; }

enum SV { HUB = 'hub', PIN = 'pin', PRE = 'pre', QUIZ = 'quiz', RESULTS = 'results', DASH = 'dash', REVIEW = 'review', HISTORY = 'history' }

interface GameSession { id: string; player: string; score: number; wrong: number; earnings: number; timestamp: number; results?: DivQuestionResult[]; }
...
export const DivideView: React.FC<DivideViewProps> = ({ onBack }) => {
    ...
    const syncWithCloud = useCallback(async () => {
        const { data, error } = await supabase.from('divide_logs').select('*').order('played_at', { ascending: false }).limit(500);
    ...
    useEffect(() => {
        syncWithCloud();
        const ch = supabase.channel('divide_logs_live').on('postgres_changes', { event: '*', schema: 'public', table: 'divide_logs' }, () => syncWithCloud()).subscribe();
    ...
    const hasPlayedToday = useCallback((player: string | null) => {
        ...
        return localStorage.getItem(`divide_attempt_${player}_${todayIST}`) === 'started';
    ...
    const startQuiz = () => {
        ...
        if (selectedUser) { const todayIST = getISTDateKey(getEffectiveDate()); localStorage.setItem(`divide_attempt_${selectedUser}_${todayIST}`, 'started'); }
    ...
    const masterQuestionHistory = useMemo(() => {
        const all: Array<DivQuestionResult & { player: string; timestamp: number }> = [];
    ...
    switch (subView) {
        case SV.HUB: return <DivideHub onBack={onBack} onNavigate={v => setSubView(v as SV)} onUserSelect={u => { setSelectedUser(u); setSubView(isPinEntryEnabled ? SV.PIN : SV.PRE); }} isMarketWorkingDay={isMarketOpenDay()} dateOverride={dateOverride} isWeekend={getEffectiveDate().getDay() === 0 || getEffectiveDate().getDay() === 6} isPublicHoliday={isMarketHoliday(getISTDateKey(getEffectiveDate()))} hasPlayedToday={hasPlayedToday} isUserLocked={u => getUserProfile(u)?.is_locked || false} />;
        case SV.PIN: return <DividePinEntry selectedUser={selectedUser} onSuccess={() => setSubView(SV.PRE)} onBack={() => setSubView(SV.HUB)} />;
        case SV.PRE: return <DividePreEntry selectedUser={selectedUser} isPlayed={hasPlayedToday(selectedUser)} isMarketWorking={isMarketOpenDay()} todaySession={getTodaySession(selectedUser)} onStart={startQuiz} onReview={s => { setSessionResults(s.results); setFinalScore(s.score); setFinalWrong(s.wrong); setFinalSessionEarnings(s.earnings); setSubView(SV.REVIEW); }} onBack={() => setSubView(SV.HUB)} />;
        case SV.QUIZ: return <DivideQuiz currentQ={questions[currentIndex]} currentIndex={currentIndex} timeLeft={timeLeft} score={score} displayInput={displayInput} onKeyClick={handleKeyClick} />;
        case SV.RESULTS: return <DivideResults finalSessionEarnings={finalSessionEarnings} finalScore={finalScore} finalWrong={finalWrong} onReview={() => setSubView(SV.REVIEW)} onExit={() => setSubView(SV.HUB)} />;
        case SV.DASH: return <DivideDashboard ayaanTotal={ayaanTotal} riyaanTotal={riyaanTotal} groupedHistory={groupedHistory} onBack={() => setSubView(SV.HUB)} />;
        case SV.REVIEW: return <DivideReview sessionResults={sessionResults} onBack={() => setSubView(SV.RESULTS)} />;
        case SV.HISTORY: return <DivideHistory masterQuestionHistory={masterQuestionHistory} historyFilter={historyFilter} setHistoryFilter={setHistoryFilter} onBack={() => setSubView(SV.HUB)} />;
        default: return null;
    }
};
