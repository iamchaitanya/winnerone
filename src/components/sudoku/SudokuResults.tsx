...
import { IndianRupee, Check, XCircle } from 'lucide-react';
...
export const SudokuResults: React.FC<SudokuResultsProps> = ({
    finalScore, finalWrong, finalEarnings, onExit
}) => {
    return (
        <div className="min-h-screen bg-slate-50 dark:bg-slate-950 flex flex-col items-center justify-center p-6 animate-in zoom-in-95 duration-500 text-center">
            ...
            <h2 className="text-3xl font-black text-slate-900 dark:text-white mb-2 tracking-tight">Sudoku Results</h2>
            <div className={`text-6xl font-black mb-12 tabular-nums ${finalEarnings >= 0 ? 'text-emerald-500' : 'text-rose-500'}`}>
                ₹{finalEarnings}
            </div>
            <div className="grid grid-cols-2 gap-4 mb-8 w-full max-w-xs mx-auto">
                <div className="p-4 bg-white dark:bg-slate-900 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-800 text-center">
                    ...
                    <p className="text-[10px] font-bold text-slate-400 uppercase mt-1">Correct</p>
                </div>
                <div className="p-4 bg-white dark:bg-slate-900 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-800 text-center">
                    <XCircle size={20} className="mx-auto mb-1 text-rose-500" />
                    <p className="text-2xl font-black text-rose-500">-{finalWrong}</p>
                    <p className="text-[10px] font-bold text-slate-400 uppercase mt-1">Wrong</p>
                </div>
            </div>
            <button onClick={onExit} className="w-full max-w-sm h-16 bg-slate-900 dark:bg-white text-white dark:text-slate-900 rounded-2xl font-black text-lg tracking-widest uppercase shadow-xl active:scale-95 transition-all mx-auto block">
                EXIT
            </button>
        </div>
    );
