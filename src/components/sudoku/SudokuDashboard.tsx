...
interface SudokuDashboardProps {
    ayaanTotal: number;
    riyaanTotal: number;
...
export const SudokuDashboard: React.FC<SudokuDashboardProps> = ({
    ayaanTotal, riyaanTotal, groupedHistory, onBack
}) => {
    return (
        <div className="min-h-screen bg-slate-50 dark:bg-slate-950 p-6 animate-in fade-in duration-500">
            <header className="flex items-center gap-4 mb-8">
                ...
                <h1 className="text-xl font-bold text-slate-900 dark:text-white">Sudoku Dashboard</h1>
            </header>
            ...
            <div className="max-w-md mx-auto space-y-3">
                ...
                {groupedHistory.map(record => (
                    <div key={record.dateKey} className="p-4 bg-white dark:bg-slate-900 rounded-2xl border border-slate-100 dark:border-slate-800 shadow-sm">
                        <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">{record.displayDate}</p>
                        <div className="grid grid-cols-2 gap-4">
                            <div className="text-center">
                                <p className="text-[10px] font-bold text-indigo-400 uppercase">Ayaan</p>
                                <p className={`font-black text-lg ${record.ayaanEarnings !== null ? (record.ayaanEarnings >= 0 ? 'text-emerald-500' : 'text-rose-500') : 'text-slate-300'}`}>
                                    {record.ayaanEarnings !== null ? `₹${record.ayaanEarnings}` : '—'}
                                </p>
                            </div>
                            <div className="text-center">
                                <p className="text-[10px] font-bold text-rose-400 uppercase">Riyaan</p>
                                <p className={`font-black text-lg ${record.riyaanEarnings !== null ? (record.riyaanEarnings >= 0 ? 'text-emerald-500' : 'text-rose-500') : 'text-slate-300'}`}>
                                    {record.riyaanEarnings !== null ? `₹${record.riyaanEarnings}` : '—'}
                                </p>
                            </div>
                        </div>
                    </div>
