...
interface Multiplication25PreEntryProps {
...
export const Multiplication25PreEntry: React.FC<Multiplication25PreEntryProps> = ({
    ...
    return (
        <div className="min-h-screen bg-slate-50 dark:bg-slate-950 flex flex-col items-center justify-center p-6 animate-in zoom-in-95 duration-300 text-center">
            <div className="w-24 h-24 bg-white dark:bg-slate-900 rounded-[2rem] shadow-2xl flex items-center justify-center mb-8 border border-slate-100 dark:border-slate-800">
                {selectedUser === 'Ayaan' ? <User size={48} className="text-teal-600" /> : <Users size={48} className="text-cyan-600" />}
            </div>
            ...
            {isPlayed ? (
                <>
                    ...
                    <button onClick={() => todaySession && onReview(todaySession)} className="w-full max-w-xs h-20 bg-slate-900 dark:bg-white text-white dark:text-slate-900 rounded-[2rem] font-black text-xl flex items-center justify-center gap-4 transition-all hover:scale-105 active:scale-95 shadow-xl mx-auto block">
                ...
                <>
                    <div className="w-full max-w-xs bg-white dark:bg-slate-900 rounded-2xl border border-slate-100 dark:border-slate-800 p-5 mb-8 text-left shadow-sm">
                        ...
                        <ul className="space-y-2 text-sm text-slate-600 dark:text-slate-300 font-medium">
                            <li>• Multiply & divide numbers from 1–25 tables</li>
                            <li>• Mixed multiplication and division questions</li>
                            <li>• 100 seconds to answer as many as possible</li>
                            <li>• <span className="text-emerald-500 font-bold">+₹{settings.multiplication25Multiplier}</span> correct · <span className="text-rose-500 font-bold">−₹{settings.multiplication25Multiplier}</span> wrong</li>
