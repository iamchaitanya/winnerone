...
interface Multiplication25DashboardProps {
    ayaanTotal: number; riyaanTotal: number;
    groupedHistory: DailyRecord[]; onBack: () => void;
}

export const Multiplication25Dashboard: React.FC<Multiplication25DashboardProps> = ({ ayaanTotal, riyaanTotal, groupedHistory, onBack }) => {
    const [isHistoryCollapsed, setIsHistoryCollapsed] = useState(false);
    const isAyaanLeading = ayaanTotal >= riyaanTotal;
    const leader = isAyaanLeading ? { name: 'Ayaan', total: ayaanTotal, color: 'teal', icon: User } : { name: 'Riyaan', total: riyaanTotal, color: 'cyan', icon: Users };
    const runner = isAyaanLeading ? { name: 'Riyaan', total: riyaanTotal, color: 'cyan', icon: Users } : { name: 'Ayaan', total: ayaanTotal, color: 'teal', icon: User };
                                ...
                                <div className="overflow-x-auto">
                                    <table className="w-full text-left table-auto">
                                        ...
                                        <tbody className="divide-y divide-slate-50 dark:divide-slate-800">
                                            {groupedHistory.length > 0 ? groupedHistory.map((record) => (
                                                <tr key={record.dateKey} className="hover:bg-slate-50/50 dark:hover:bg-slate-800/30 transition-colors">
                                                    <td className="px-5 py-5 whitespace-nowrap"><span className="text-[11px] text-slate-900 dark:text-slate-200 font-bold tabular-nums">{record.displayDate}</span></td>
                                                    <td className="px-5 py-5 text-center">
                                                        {record.ayaanEarnings !== null ? <div className="flex flex-col items-center"><span className={`text-sm font-black tabular-nums ${record.ayaanEarnings >= 0 ? 'text-emerald-500' : 'text-rose-500'}`}>{record.ayaanEarnings >= 0 ? '+' : ''}{record.ayaanEarnings.toLocaleString()}</span><span className="text-[9px] text-slate-400 font-medium tabular-nums whitespace-nowrap">{record.ayaanTime}</span></div> : <span className="text-slate-200 dark:text-slate-800">—</span>}
                                                    </td>
                                                    <td className="px-5 py-5 text-center">
                                                        {record.riyaanEarnings !== null ? <div className="flex flex-col items-center"><span className={`text-sm font-black tabular-nums ${record.riyaanEarnings >= 0 ? 'text-emerald-500' : 'text-rose-500'}`}>{record.riyaanEarnings >= 0 ? '+' : ''}{record.riyaanEarnings.toLocaleString()}</span><span className="text-[9px] text-slate-400 font-medium tabular-nums whitespace-nowrap">{record.riyaanTime}</span></div> : <span className="text-slate-200 dark:text-slate-800">—</span>}
                                                    </td>
                                                </tr>
                                            )) : <tr><td colSpan={3} className="px-5 py-16 text-center"><div className="flex flex-col items-center gap-2"><History size={32} className="text-slate-200 dark:text-slate-800" /><p className="text-slate-400 text-xs font-medium italic">No entries in history.</p></div></td></tr>}
                                        </tbody>
                                    </table>
                                </div>
                            </div>
