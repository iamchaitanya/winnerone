...
interface HistoryEntry extends Mul25QuestionResult { player: string; timestamp: number; }

interface Multiplication25HistoryProps {
    masterQuestionHistory: HistoryEntry[];
    historyFilter: 'Ayaan' | 'Riyaan';
    setHistoryFilter: (f: 'Ayaan' | 'Riyaan') => void;
    onBack: () => void;
}

export const Multiplication25History: React.FC<Multiplication25HistoryProps> = ({
    masterQuestionHistory, historyFilter, setHistoryFilter, onBack
}) => (
            ...
            <div className="flex items-center gap-2 p-1.5 bg-slate-100 dark:bg-slate-900 rounded-2xl w-full">
                {(['Ayaan', 'Riyaan'] as const).map(filter => (
                    <button key={filter} onClick={() => setHistoryFilter(filter)}
                        className={`flex-1 py-3 rounded-xl text-xs font-black uppercase tracking-widest transition-all ${historyFilter === filter ? 'bg-white dark:bg-slate-800 text-teal-600 dark:text-teal-400 shadow-sm' : 'text-slate-400 hover:text-slate-600'}`}>
                        {filter}
                    </button>
                ))}
            </div>
            ...
            <div className="overflow-x-auto">
                <table className="w-full text-left table-auto">
                    ...
                    <tbody className="divide-y divide-slate-50 dark:divide-slate-800">
                        {masterQuestionHistory.length > 0 ? masterQuestionHistory.map((res, idx) => {
