...
export const VocabResults: React.FC<{ title: string; finalScore: number; finalWrong: number; finalEarnings: number; onExit: () => void; }> = ({ title, finalScore, finalWrong, finalEarnings, onExit }) => (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 flex flex-col items-center justify-center p-6 animate-in zoom-in-95 duration-500 text-center">
        ...
        <div className="grid grid-cols-2 gap-4 mb-8 w-full max-w-xs mx-auto">
            <div className="p-4 bg-white dark:bg-slate-900 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-800 text-center"><Check size={20} className="mx-auto mb-1 text-emerald-500" /><p className="text-2xl font-black text-slate-900 dark:text-white">+{finalScore}</p><p className="text-[10px] font-bold text-slate-400 uppercase mt-1">Correct</p></div>
            <div className="p-4 bg-white dark:bg-slate-900 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-800 text-center"><XCircle size={20} className="mx-auto mb-1 text-rose-500" /><p className="text-2xl font-black text-rose-500">-{finalWrong}</p><p className="text-[10px] font-bold text-slate-400 uppercase mt-1">Wrong</p></div>
        </div>
