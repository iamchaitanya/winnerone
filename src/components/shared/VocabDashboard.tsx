...
export const VocabDashboard: React.FC<VocabDashboardProps> = ({
    title,
    ayaanTotal,
    riyaanTotal,
    groupedHistory,
        ...
        <div className="min-h-screen bg-slate-50 dark:bg-slate-950 p-6 animate-in slide-in-from-right duration-300 overflow-x-hidden">
            <div className="max-w-2xl mx-auto w-full">
                ...
                <div className="flex flex-col items-center w-full">
                    {/* Visual Leaderboard */}
                    <div className="relative flex items-center justify-center w-full h-64 sm:h-80 mb-6 font-sans">
                        <div className="flex items-center justify-center">
                            {/* Leader Circle */}
                            <div className={`relative z-20 w-44 h-44 sm:w-56 sm:h-56 md:w-64 md:h-64 rounded-full bg-white dark:bg-slate-900 border-[6px] ${leader.color === 'indigo' ? 'border-indigo-500' : 'border-rose-500'} flex flex-col items-center justify-center shadow-2xl animate-in zoom-in duration-700 overflow-hidden`}>
                                ...
                                <leader.icon size={32} className={`${leader.color === 'indigo' ? 'text-indigo-500' : 'text-rose-500'} mb-1 mt-4 sm:mt-6`} />
                            ...
                            {/* Runner Circle */}
                            <div className={`relative z-10 -ml-10 sm:-ml-12 w-32 h-32 sm:w-40 sm:h-40 md:w-48 md:h-48 rounded-full bg-white dark:bg-slate-900 border-2 ${runner.color === 'indigo' ? 'border-indigo-300' : 'border-rose-300'} flex flex-col items-center justify-center shadow-xl animate-in zoom-in duration-1000 delay-300 overflow-hidden`}>
                                <runner.icon size={20} className={`${runner.color === 'indigo' ? 'text-indigo-400' : 'text-rose-400'} mb-1`} />
                    ...
                    <div className="w-full mt-4 mb-24 px-1">
                        ...
                        {!isHistoryCollapsed && (
                            <div className="bg-white dark:bg-slate-900 rounded-3xl border border-slate-100 dark:border-slate-800 shadow-sm overflow-hidden animate-in slide-in-from-top-2 duration-300 w-full mb-12">
