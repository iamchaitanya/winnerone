...
interface MultiplyReviewProps {
    sessionResults: MulQuestionResult[];
    onBack: () => void;
}

export const MultiplyReview: React.FC<MultiplyReviewProps> = ({ sessionResults, onBack }) => {
    return (
                    ...
                    <div key={i} className={`flex items-center justify-between p-4 rounded-2xl border ${r.isCorrect ? 'bg-emerald-50/50 dark:bg-emerald-900/10 border-emerald-100 dark:border-emerald-800/30' : 'bg-rose-50/50 dark:bg-rose-900/10 border-rose-100 dark:border-rose-800/30'}`}>
                        <div className="flex items-center gap-3">
                            ...
                            <span className="text-sm font-black text-slate-900 dark:text-white tabular-nums">
                                {r.num1} <span className="text-pink-500">×</span> {r.num2}
                            </span>
                        </div>
                        <div className="flex items-center gap-4 text-sm font-black tabular-nums">
                            <span className={r.isCorrect ? 'text-emerald-600 dark:text-emerald-400' : 'text-rose-600 dark:text-rose-400'}>{r.userAnswer}</span>
                            {!r.isCorrect && <span className="text-emerald-600 dark:text-emerald-400">{r.answer}</span>}
                        </div>
                    </div>
                ))}
            </div>
