...
import { MulQuestionResult } from '../../hooks/useMultiplyEngine';

interface MultiplyHistoryProps {
    masterQuestionHistory: Array<MulQuestionResult & { player: string; timestamp: number }>;
    historyFilter: 'Ayaan' | 'Riyaan';
    setHistoryFilter: (f: 'Ayaan' | 'Riyaan') => void;
    onBack: () => void;
}

export const MultiplyHistory: React.FC<MultiplyHistoryProps> = ({ masterQuestionHistory, historyFilter, setHistoryFilter, onBack }) => {
    return (
        <div className="min-h-screen bg-slate-50 dark:bg-slate-950 p-6 animate-in fade-in duration-300">
            <header className="flex items-center gap-4 mb-6">
                ...
                <h1 className="text-xl font-bold text-slate-900 dark:text-white">Multiply History</h1>
            </header>
            ...
            <div className="max-w-lg mx-auto mb-6 grid grid-cols-2 gap-2">
                {(['Ayaan', 'Riyaan'] as const).map(name => (
                    <button key={name} onClick={() => setHistoryFilter(name)}
                        className={`py-3 rounded-2xl font-black text-sm uppercase tracking-widest transition-all text-center ${historyFilter === name ? 'bg-pink-500 text-white shadow-lg shadow-pink-500/20' : 'bg-white dark:bg-slate-900 text-slate-400 border border-slate-100 dark:border-slate-800'}`}>
                        {name}
                    </button>
                ))}
            </div>
                                    ...
                                    <tr className={`transition-colors ${res.isCorrect ? 'bg-emerald-50/30 dark:bg-emerald-900/10 hover:bg-emerald-50 dark:hover:bg-emerald-900/20' : 'bg-rose-50/30 dark:bg-rose-900/10 hover:bg-rose-50 dark:hover:bg-rose-900/20'}`}>
                                        <td className="px-4 py-3 whitespace-nowrap">
                                            <span className="text-sm font-black text-slate-900 dark:text-slate-100 tabular-nums">
                                                {res.num1} <span className="text-pink-500">×</span> {res.num2}
                                            </span>
                                        </td>
                                        <td className="px-3 py-3 text-center"><span className={`text-sm font-black tabular-nums ${res.isCorrect ? 'text-emerald-600 dark:text-emerald-400' : 'text-rose-600 dark:text-rose-400'}`}>{res.userAnswer}</span></td>
                                        <td className="px-3 py-3 text-center"><span className="text-sm font-black text-emerald-600 dark:text-emerald-400 tabular-nums">{res.answer}</span></td>
