...
interface WordPowerHubProps { onBack: () => void; onNavigate: (view: any) => void; onUserSelect: (user: string) => void; isMarketWorkingDay: boolean; dateOverride: string | null; isWeekend: boolean; isPublicHoliday: boolean; hasPlayedToday: (user: string) => boolean; isUserLocked: (user: string) => boolean; }

export const WordPowerHub: React.FC<WordPowerHubProps> = ({ onBack, onNavigate, onUserSelect, isMarketWorkingDay, dateOverride, isWeekend, isPublicHoliday, hasPlayedToday, isUserLocked }) => {
        ...
        <div className="min-h-screen bg-slate-50 dark:bg-slate-950 p-6 animate-in fade-in duration-500">
            <header className="flex items-center justify-between mb-8">
                <div className="flex items-center gap-4">
                    ...
                    <div className="flex flex-col">
                        <div className="flex items-center gap-2"><h1 className="text-xl font-bold text-slate-900 dark:text-white leading-tight">Word Power Hub</h1>{!isMarketWorkingDay && <span className="px-2 py-0.5 bg-rose-100 dark:bg-rose-900/30 text-rose-600 dark:text-rose-400 text-[10px] font-black uppercase rounded-full tracking-wider border border-rose-200/50 dark:border-rose-800/50">Market Closed</span>}</div>
            ...
            <section className="flex flex-col gap-4 max-w-md mx-auto">
                {(['Ayaan', 'Riyaan'] as const).map(name => {
                    const played = name === 'Ayaan' ? ayaanPlayed : riyaanPlayed; const locked = isUserLocked(name); const color = name === 'Ayaan' ? 'indigo' : 'rose'; return (
                        <button key={name} onClick={() => onUserSelect(name)} className={`flex flex-row items-center px-6 h-24 bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-3xl shadow-xl transition-all group hover:border-${color}-400 active:scale-[0.98]`}>
                            <div className="flex items-center gap-6 w-full">
                                <div className={`p-4 bg-${color}-50 dark:bg-${color}-900/30 text-${color}-600 dark:text-${color}-400 rounded-2xl group-hover:scale-110 transition-transform`}>{locked ? <Lock size={32} className="text-rose-500" /> : name === 'Ayaan' ? <User size={32} /> : <Users size={32} />}</div>
                                <div className="flex flex-col items-start text-left">
                                    <span className={`font-black text-2xl tracking-tighter uppercase leading-none ${locked ? 'text-rose-500' : 'text-slate-900 dark:text-white'}`}>{name.toUpperCase()} {locked && '(LOCKED)'}</span>
                                    {played && !locked && <span className="text-[10px] font-bold text-emerald-500 uppercase mt-1 flex items-center gap-1"><Check size={10} /> Completed Today</span>}
                                </div>
                            </div>
                        </button>
                    );
