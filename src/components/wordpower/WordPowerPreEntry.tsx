...
export const WordPowerPreEntry: React.FC<{ selectedUser: string | null; isPlayed: boolean; isMarketWorking: boolean; onStart: () => void; onBack: () => void; }> = ({ selectedUser, isPlayed, isMarketWorking, onStart, onBack }) => (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 flex flex-col items-center justify-center p-6 animate-in zoom-in-95 duration-300 text-center">
        ...
        {isPlayed ? <div className="bg-emerald-50 dark:bg-emerald-900/20 px-6 py-4 rounded-3xl border border-emerald-100 dark:border-emerald-800/50 mb-12 flex items-center gap-3"><Check size={20} className="text-emerald-500" /><p className="text-emerald-600 dark:text-emerald-400 font-bold text-sm uppercase tracking-wide">Finished today's Word Power!</p></div>
                ...
                : <>
                    <div className="w-full max-w-xs bg-white dark:bg-slate-900 rounded-2xl border border-slate-100 dark:border-slate-800 p-5 mb-8 text-left shadow-sm">
                        ...
                        <ul className="space-y-2 text-sm text-slate-600 dark:text-slate-300 font-medium">
                            <li>• 1 Root word with meaning shown for 15 seconds</li>
                            <li>• 3 derivative word questions (15s each)</li>
                            <li>• <span className="text-emerald-500 font-bold">+Time</span> correct · <span className="text-rose-500 font-bold">−Time</span> wrong</li>
                            <li>• 0 pts for unanswered questions</li>
                        </ul>
                    </div>
