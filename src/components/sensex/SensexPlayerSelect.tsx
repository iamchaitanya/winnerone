...
interface SensexPlayerSelectProps {
  selectedUser: string | null;
  isPlayed: boolean;
  isPickWindow: boolean;
  isMarketWorking: boolean;
  onNavigate: (view: 'PICK' | 'RESULTS' | 'HUB') => void;
  onBack: () => void;
}

export const SensexPlayerSelect: React.FC<SensexPlayerSelectProps> = ({
  selectedUser,
  isPlayed,
  isPickWindow,
  isMarketWorking,
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 flex flex-col items-center justify-center p-6 animate-in zoom-in-95 duration-300 text-center">
      ...
      <h2 className="text-4xl font-black text-slate-900 dark:text-white mb-2 tracking-tight uppercase">Sensex Pick</h2>
      ...
      {isPlayed ? (
        <>
          <div className="bg-emerald-50 dark:bg-emerald-900/20 px-6 py-4 rounded-3xl border border-emerald-100 dark:border-emerald-800/50 mb-12 flex items-center gap-3">
            ...
            <p className="text-emerald-600 dark:text-emerald-400 font-bold text-sm uppercase">Prediction Saved!</p>
          </div>
        ...
        <>
          {!isMarketWorking ? (
            <div className="bg-amber-50 dark:bg-amber-900/20 px-8 py-6 rounded-[2rem] border border-amber-100 dark:border-amber-800/50 mb-12 max-w-xs">
              ...
              <p className="text-[10px] font-bold text-slate-400 uppercase mt-2">No predictions allowed today</p>
            </div>
          ) : !isPickWindow ? (
            <div className="bg-rose-50 dark:bg-rose-900/20 px-8 py-6 rounded-[2rem] border border-rose-100 dark:border-rose-800/50 mb-12 max-w-xs">
              ...
              <p className="text-rose-600 dark:text-rose-400 font-black text-sm uppercase tracking-wide">Window Closed</p>
              <p className="text-[10px] font-bold text-slate-400 uppercase mt-2">Picks must be made before 9:00 AM IST</p>
            </div>
          ) : (
            <>
              <p className="text-slate-400 font-medium mb-12 uppercase tracking-widest text-xs px-8 leading-relaxed">Predict UP or DOWN before 9 AM. Results settled after 3:30 PM.</p>
              <button onClick={() => onNavigate('PICK')} className="w-full max-w-xs h-20 bg-slate-900 dark:bg-white text-white dark:text-slate-900 rounded-[2rem] font-black text-xl flex items-center justify-center gap-4 shadow-xl mx-auto block">
                <Play size={24} fill="currentColor" /> START PREDICTION
              </button>
            </>
          )}
        </>
