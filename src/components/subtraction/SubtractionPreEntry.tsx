...
interface SubtractionPreEntryProps {
...
export const SubtractionPreEntry: React.FC<SubtractionPreEntryProps> = ({
    selectedUser,
    isPlayed,
    isMarketWorking,
    todaySession,
                ...
                <>
                    <div className="w-full max-w-xs bg-white dark:bg-slate-900 rounded-2xl border border-slate-100 dark:border-slate-800 p-5 mb-8 text-left shadow-sm">
                        ...
                        <ul className="space-y-2 text-sm text-slate-600 dark:text-slate-300 font-medium">
                            <li>• Subtract two 2-digit numbers (10–99)</li>
                            <li>• Answers can be negative</li>
                            <li>• 100 seconds to answer as many as possible</li>
                            <li>• <span className="text-emerald-500 font-bold">+₹{settings.subtractionMultiplier}</span> correct · <span className="text-rose-500 font-bold">−₹{settings.subtractionMultiplier}</span> wrong</li>
