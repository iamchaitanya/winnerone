...
import { ArrowLeft, User, Users, IndianRupee, Crown, History, ChevronRight, ChevronDown, Trophy, Medal } from 'lucide-react';

interface NiftyLeaderboardProps {
  ayaanTotal: number;
  riyaanTotal: number;
  groupedHistory: any[];
  onBack: () => void;
...
export const NiftyLeaderboard: React.FC<NiftyLeaderboardProps> = ({
  ayaanTotal,
  riyaanTotal,
  groupedHistory,
  onBack
                        ...
                        groupedHistory.map((record: any) => (
                          <tr key={record.date} className="hover:bg-slate-50/50 dark:hover:bg-slate-800/30 transition-colors">
                            <td className="px-5 py-5 whitespace-nowrap">
                              <span className="text-[11px] text-slate-900 dark:text-slate-200 font-bold tabular-nums">
