...
import { MulQuestionResult } from '../../hooks/useMultiplicationEngine';

interface HistoryEntry extends MulQuestionResult {
    player: string;
    timestamp: number;
}

interface MultiplicationHistoryProps {
...
export const MultiplicationHistory: React.FC<MultiplicationHistoryProps> = ({
    ...
    return (
        <div className="min-h-screen bg-slate-50 dark:bg-slate-950 p-6 animate-in slide-in-from-right duration-300">
            <header className="flex flex-col gap-6 mb-8 max-w-lg mx-auto w-full">
                ...
                {/* Filter tabs — full width, matches SubtractionHistory */}
                                ...
                                masterQuestionHistory.map((res, idx) => {
                                    ...
                                    return (
                                        <React.Fragment key={idx}>
                                            ...
                                            <tr className={`transition-colors ${res.isCorrect
                                                ...
                                                <td className="px-4 py-3 whitespace-nowrap">
                                                    <span className="text-sm font-black text-slate-900 dark:text-slate-100 tabular-nums">
                                                        {res.operand1}{' '}
                                                        <span className={res.type === 'multiply' ? 'text-violet-500' : 'text-purple-500'}>{sym}</span>
                                                        {' '}{res.operand2}
                                                    </span>
                                                </td>
