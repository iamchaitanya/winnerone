...
interface MultiplicationReviewProps {
    sessionResults: MulQuestionResult[];
    onBack: () => void;
}

export const MultiplicationReview: React.FC<MultiplicationReviewProps> = ({ sessionResults, onBack }) => {
    ...
    return (
        <div className="min-h-screen bg-slate-50 dark:bg-slate-950 flex flex-col animate-in slide-in-from-right duration-300">
            <header className="flex items-center gap-4 p-6 pb-0">
                <button onClick={onBack} className="p-2 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-full text-slate-900 dark:text-white transition-colors">
                    <ArrowLeft size={24} />
                </button>
                <div>
                                    ...
                                    <tr key={i} className={`border-b border-slate-50 dark:border-slate-800/50 ${res.isCorrect ? '' : 'bg-rose-50/40 dark:bg-rose-900/10'}`}>
                                        <td className="px-4 py-4">
                                            <span className="text-sm font-black text-slate-900 dark:text-slate-100 tabular-nums">
                                                {res.operand1} <span className={res.type === 'multiply' ? 'text-violet-500' : 'text-purple-500'}>{sym}</span> {res.operand2}
                                            </span>
                                        </td>
