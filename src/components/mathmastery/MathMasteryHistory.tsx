...
interface MathMasteryHistoryProps {
    history: any[];
    historyFilter: string;
    setHistoryFilter: (filter: string) => void;
    masterQuestionHistory: Array<MathMasteryResult & { player: string; timestamp: number }>;
    onBack: () => void;
}

export const MathMasteryHistory: React.FC<MathMasteryHistoryProps> = ({
    historyFilter,
    setHistoryFilter,
        ...
        <div className="min-h-screen bg-slate-50 dark:bg-slate-950 p-6 animate-in slide-in-from-right duration-300">
            <header className="flex flex-col gap-6 mb-8 max-w-lg mx-auto w-full">
                <div className="flex items-center gap-4">
                    <button
                        onClick={onBack}
                        className="p-2 hover:bg-slate-100 dark:bg-slate-800 rounded-full transition-colors text-slate-900 dark:text-white"
                    >
                        <ArrowLeft size={24} />
                    </button>
