...
import { ArrowLeft, User, Users, BarChart2, History, Check, Lock, CalendarX, Clock, BrainCircuit } from 'lucide-react';

interface MathMasteryHubProps {
    onBack: () => void;
...
export const MathMasteryHub: React.FC<MathMasteryHubProps> = ({
        ...
        <div className="min-h-screen bg-slate-50 dark:bg-slate-950 p-6 animate-in fade-in duration-500">
                ...
                <div className="flex items-center gap-4">
                    ...
                    <div className="flex flex-col">
                        <div className="flex items-center gap-2">
                            <h1 className="text-xl font-bold text-slate-900 dark:text-white leading-tight">Math Mastery Hub</h1>
                            {!isMarketWorkingDay && (
                ...
                <div className="mb-8 max-w-md mx-auto">
                    <div className="p-5 rounded-[2rem] border bg-rose-50 dark:bg-rose-900/10 border-rose-100 dark:border-rose-800/30 flex items-center gap-4 shadow-sm">
                        ...
                        <div>
                            ...
                            <p className="text-[10px] font-bold text-slate-400 mt-0.5">Games disabled during closures.</p>
                        </div>
                    </div>
                </div>
            )}
            ...
            <section className="flex flex-col gap-4 max-w-md mx-auto">
                ...
                <button
                    ...
                    className="flex flex-row items-center px-6 h-24 bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-3xl shadow-xl transition-all group hover:border-purple-400 active:scale-[0.98]"
                >
                    <div className="flex items-center gap-6 w-full">
                ...
                <button onClick={() => onNavigate('local_dashboard')} className="flex flex-row items-center px-6 h-24 bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-3xl shadow-xl hover:border-indigo-400 transition-all active:scale-[0.98] group">
                    <div className="flex items-center gap-6">
                        <div className="p-4 bg-indigo-50 dark:bg-indigo-900/30 text-indigo-600 dark:text-indigo-400 rounded-2xl group-hover:scale-110 transition-transform"><BarChart2 size={32} /></div>
                        <span className="font-black text-slate-900 dark:text-white text-2xl tracking-tighter uppercase">DASHBOARD</span>
                    </div>
                </button>

                <button onClick={() => onNavigate('master_history')} className="flex flex-row items-center px-6 h-24 bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-3xl shadow-xl hover:border-purple-400 transition-all active:scale-[0.98] group">
                    <div className="flex items-center gap-6">
                        <div className="p-4 bg-purple-50 dark:bg-purple-900/30 text-purple-600 dark:text-purple-400 rounded-2xl group-hover:scale-110 transition-transform"><History size={32} /></div>
